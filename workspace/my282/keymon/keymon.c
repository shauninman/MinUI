/**
 * keymon.c - MY282 hardware button monitoring daemon
 *
 * Background daemon that monitors physical button presses and handles
 * system-level shortcuts on the MY282 handheld device. Provides volume
 * and brightness control through hardware button combinations:
 * - MENU+PLUS/MINUS: Adjust brightness
 * - PLUS/MINUS alone: Adjust volume
 *
 * Uses different input event codes than SDL (CODE_MENU can be 1 or 354).
 * Monitors two separate input devices (event0 and event3).
 *
 ...
 *
 * @note Uses event0 and event3 input devices
 */
